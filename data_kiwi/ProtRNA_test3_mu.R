...
# attach(poids)
pdf("testValeurFictives_test2.pdf")
t<-c(0,7,14,21,28,35,42,49,55)
w<-c(0.25,25,60,70,80,88,95,98,100)
...
# curve<-sigmoide(t,initialisation)
# plot(t,curve)
# points(t,w,col="red")
...
#
valp=coef(model_sig)
plot(t,sigmoide(t,valp),"l",ylim = c(0,100))
points(t,w,col="red")
##############################################################
# choix rna courbe
rna<-function(t) {
...
list_par<-expand.grid(c(0.05,5,500),c(0.005,0.5,50),c(100,10000,1000000))
colnames(list_par)<-c("ks","kd","ratio")
...
#   cont<-cont+1
# }
for (i in seq(nrow(list_par))){
  newlist<-list("start_prot"=list_par[i,"ratio"]*test_rna[1],"ks"=list_par[i,"ks"]*4.646001e-05,"kd"=list_par[i,"kd"])
  listPar2[[cont]]=newlist
  cont<-cont+1
}
...
for (i in seq(1,length(listPar2))){
  sol=ode(y=listPar2[[i]][["start_prot"]],times=ti,func=fmu,parms=c(ks=listPar2[[i]][["ks"]],kd=listPar2[[i]][["kd"]]))
  solb<-matrix(nrow=length(sol[,2]),ncol=2)
  solb[,1]=sol[,1]
  bruit=min(sqrt(mean(sol[,2])/6),0.5)
  val_prot<-Noisify(sol[,2],bruit)
...
# res_list<-lapply(listPar, function(el){
  ...
  # t1=seq(0,55,dt)
  # ti=sort(c(t1,t1,t1))
  sol=ode(y=listPar2[[j]][["start_prot"]],times=ti,func=fmu,parms=c(ks=el[["ks"]],kd=el[["kd"]]),method = "ode45")
  ...
  #ksmin=ks_vrai*0.25#
  ksmin=4.5e-3*mean(test_rna)/mean(sol)^2
  ksmax=1440*mean(test_rna)
  #kdmin=kd_vrai*0.25
  kdmin=4.5e-3
  kdmax=50
  lb=c(ksmin,kdmin,0);ub=c(ksmax,kdmax,2*mean(solb[1:3,2]))
  ...
  fit<-nls(solb$p~ode(y=p00,times=solb[,1],func=fmu,parms=c(ks=ks,kd=kd))[,2],data=list(solb),start=initialisation,trace=TRUE,algorithm="port",control=list(minFactor=1e-6,maxiter=1000,warnOnly=F,tol=1e-6),lower=lb,upper=ub)
  fit2<-nlsLM(solb$p~ode(y=p00,times=solb[,1],func=fmu,parms=c(ks=ks,kd=kd))[,2],data=list(solb),start=initialisation,lower=lb,upper=ub,trace=TRUE)
  # summary(fit)
  # summary(fit2)
  ...
  par(new=TRUE)
  theo_exact<-ode(y=listPar2[[j]][["start_prot"]],times=sol[,1],func=fmu,parms=c(ks=res[["ks"]],kd=res[["kd"]]))
